...
const intent = await (
  await request("/api/v1/fs/upload-intents", {
    body: { path: `${prefix}/artifact.html`, contentType: "text/html", size: Buffer.byteLength(html) },
  })
).json();
...
checks++;
await request(`/api/v1/fs/uploads/${intent.uploadId}/commit`, { token: bob, body: { etag: "etag" }, status: 404 });
...
const { share } = await (
  await request("/api/v1/shares", { body: { path: `${prefix}/artifact.html`, ttl: "15m" } })
).json();
...
const device = await (
  await request("/api/v1/device/start", { token: null, body: { clientName: "local security smoke" } })
).json();
...
const approved = await (
  await request("/api/v1/device/poll", { token: null, body: { deviceCode: device.deviceCode } })
).json();
...
console.log(
  `Passed ${checks} local HTTP checks: auth, owner isolation, literal paths, upload/commit/replay, safe shares/revocation, device login/replay.`,
);
