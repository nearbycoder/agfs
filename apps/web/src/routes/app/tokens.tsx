    ...
    <div className="space-y-6">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_380px]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create long-lived AGFS tokens for automation, keep labels readable, and revoke anything you no longer trust.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <form className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_160px_auto]" onSubmit={handleCreate}>
              <div className="space-y-2">
                <label className="section-label" htmlFor="token-label">
                  Label
                </label>
                <Input id="token-label" onChange={(event) => setLabel(event.target.value)} placeholder="Build runner" value={label} />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50/70 p-4 dark:border-zinc-800 dark:bg-zinc-900/60">
                ...
                <p className="mt-2 text-sm leading-6 text-zinc-500 dark:text-zinc-400">AGFS stores token hashes in D1 and only reveals the raw token once at creation time.</p>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50/70 p-4 dark:border-zinc-800 dark:bg-zinc-900/60">
                <p className="section-label">Current count</p>
                <p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-zinc-950 dark:text-zinc-50">{tokens.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {secret ? (
              <div className="space-y-4">
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-4 font-mono text-sm leading-6 text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900/70 dark:text-zinc-200">{secret}</div>
              ...
              <p className="text-sm leading-6 text-zinc-500 dark:text-zinc-400">Create a token and the latest value will appear here for a one-time copy.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Keep labels clear so automation hosts are easy to identify and retire when they are no longer needed.</CardDescription>
        </CardHeader>
        <CardContent>
          {tokens.length === 0 ? (
              ...
              <TableBody>
                {tokens.map((token) => (
                  <TableRow key={token.id}>
                    ...
                    <TableCell className="font-mono text-xs text-zinc-500 dark:text-zinc-400">{token.prefix}</TableCell>
                    <TableCell>
