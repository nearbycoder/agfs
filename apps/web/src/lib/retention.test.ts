...
it("migration preserves legacy scopes and atomically retains overwritten object versions", () => {
  ...
  for (const name of [
    ...
    "0002_agent_filesystem.sql",
    "0003_object_gc.sql",
    "0004_collaboration.sql",
    "0005_oauth.sql",
    "0006_reliability.sql",
  ])
    db.exec(
      readFileSync(
        new URL(
  ...
  expect(
    db.prepare("SELECT path_prefix,permissions FROM api_tokens").get(),
  ).toMatchObject({
    path_prefix: "/",
    permissions: '["read","write","delete","share","manage"]',
  });
  expect(
    db.prepare("SELECT reason,size,r2_key FROM recovery").get(),
  ).toMatchObject({
    reason: "version",
    size: 10,
    r2_key: "old",
  ...
  const usage = () => {
    ...
    return db.prepare(`SELECT ${q.sql} AS bytes`).get(...(q.params as any[]))
      ?.bytes;
  };
