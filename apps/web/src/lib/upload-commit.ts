...
export function commitUploadStatement(row: typeof entries.$inferInsert, uploadId: string, limit: number) {
  return sql`
    ...
    WHERE EXISTS (SELECT 1 FROM uploads WHERE id = ${uploadId} AND owner_id = ${row.ownerId} AND status = 'pending' AND expires_at > ${Date.now()})
      AND NOT EXISTS (SELECT 1 FROM entries WHERE owner_id = ${row.ownerId} AND path = ${row.path} AND kind != 'file')
      AND (SELECT coalesce(sum(size), 0) FROM entries WHERE owner_id = ${row.ownerId} AND kind = 'file' AND path != ${row.path}) + ${row.size}
        <= max(${limit}, (SELECT coalesce(sum(size), 0) FROM entries WHERE owner_id = ${row.ownerId} AND kind = 'file'))
