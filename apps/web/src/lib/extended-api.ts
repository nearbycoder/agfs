...
const restoreInput = z.object({ id: z.string().min(1).max(128), path: pathSchema.optional() });
export async function extendedApi(request: Request): Promise<Response | null> {
  const url = new URL(request.url);
  const path = url.pathname;
  const method = request.method;
  if (
    !["/api/v1/recovery", "/api/v1/recovery/restore", "/api/v1/activity", "/api/v1/fs/preview"].includes(path) &&
  ...
  try {
    ...
    if (path === "/api/v1/activity" && method === "GET") {
      const rows = await listActivity(auth, url.searchParams.get("cursor") ?? undefined);
      const last = rows.at(-1);
      return json({
        events: rows,
        nextCursor: rows.length === 100 && last ? `${last.createdAt.getTime()}:${last.id}` : null,
      });
    }
    if (path === "/api/v1/recovery" && method === "GET") {
      const reason = z.enum(["trash", "version"]).parse(url.searchParams.get("reason") ?? "trash");
      const cursor = z
        .string()
        .regex(/^\d+:[a-zA-Z0-9_]+$/)
        .max(256)
      ...
      const items = await listRecovery(auth, reason, url.searchParams.get("path") ?? undefined, cursor);
      const last = items.at(-1);
      return json({
        items,
        nextCursor: items.length === 200 && last ? `${last.retainedAt.getTime()}:${last.id}` : null,
      });
    }
    if (path === "/api/v1/recovery" && method === "DELETE") {
      const removedPath = await purgeRecovery(auth, z.string().min(1).max(128).parse(url.searchParams.get("id")));
    ...
    const match = /^\/api\/v1\/fs\/resumable\/([a-zA-Z0-9_-]+)(?:\/(complete|parts\/(\d+)))?$/.exec(path);
    if (match) {
      const id = match[1];
      if (!match[2] && method === "GET") return json(await multipartStatus(auth, id));
      ...
      if (match[3] && method === "PUT") return json(await putPart(auth, id, Number(match[3]), request));
    }
