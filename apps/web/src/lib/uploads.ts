...
function active(row: typeof uploads.$inferSelect) {
  if (row.expiresAt.getTime() <= Date.now() || !["pending", "completing"].includes(row.status))
...
export async function startMultipart(
  auth: RequestAuth,
  input: { path: string; contentType: string; size: number; fingerprint: string },
) {
  input = { ...input, path: normalizeAgfsPath(input.path) };
  authorize(auth, "write", input.path);
  if (!Number.isSafeInteger(input.size) || input.size <= 0 || input.size > MAX_UPLOAD_SIZE)
    throw errorResponse(400, "Resumable files must be between 1 byte and 20 GB, within your storage quota");
  ...
  const intent = await createUploadIntent(auth.user, { ...input, resumable: true });
...
export async function putPart(auth: RequestAuth, id: string, partNumber: number, request: Request) {
  const row = await authorizeUpload(auth, id);
  active(row);
  if (row.status !== "pending") throw errorResponse(409, "Upload is being completed");
  ...
  if (bytes.byteLength !== expected) throw errorResponse(400, "Invalid part size");
  const digest = Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256", bytes)))
    .map((b) => b.toString(16).padStart(2, "0"))
    .join("");
  const multipart = requireResourceBindings("FILES_BUCKET").FILES_BUCKET.resumeMultipartUpload(
    row.objectKey,
    row.multipartId!,
  );
  ...
  await db
    .insert(uploadParts)
    .values({ uploadId: id, partNumber, etag: part.etag, size: expected, digest })
    .onConflictDoUpdate({
...
export async function completeMultipart(auth: RequestAuth, id: string) {
  ...
  if (!object) object = await FILES_BUCKET.resumeMultipartUpload(row.objectKey, row.multipartId!).complete(parts);
  return { entry: await commitUpload(auth.user, id, object.etag ?? "uploaded") };
}
