...
`apps/cimd-egress` runs the bundled transport in a digest-pinned official Node image on Cloudflare Containers. It has no public route, one maximum instance, and sleeps after thirty seconds idle. Containers incur usage charges while active and have cold-start latency. Deploy it with `pnpm --filter @agfs/cimd-egress run deploy`, the production web Worker binds `CIMD_EGRESS` to `agfs-cimd-egress`. Without that binding, CIMD is disabled and DCR remains available. The public `/.well-known/agfs-conformance-client.json` document supports deployment checks without a stored client registration. Consent resolves clients through the OAuth provider, so discovered clients receive the same validation as registered clients. The service is separate because Workers' current pinned-IP TLS transport fails hostname verification; do not replace it with an unpinned fetch or disable certificate verification.
